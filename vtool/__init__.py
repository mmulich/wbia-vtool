...
from vtool.chip import (ScaleStrat, apply_filter_funcs, compute_chip,
...
from vtool.clustering2 import (AnnoyWraper, apply_grouping, apply_grouping_,
...
from vtool.coverage_grid import (COVGRID_DEFAULT, compute_subbin_to_bins_dist,
                                 get_coverage_grid_gridsearch_configs,
                                 get_subbin_xy_neighbors,
                                 gridsearch_coverage_grid,
                                 gridsearch_coverage_grid_mask,
                                 make_grid_coverage_mask, show_coverage_grid,
                                 sparse_grid_coverage,
                                 weighted_gaussian_falloff,)
from vtool.coverage_kpts import (COVKPTS_DEFAULT,
                                 get_coverage_kpts_gridsearch_configs,
                                 get_gaussian_weight_patch,
                                 gridsearch_kpts_coverage_mask, make_heatmask,
                                 make_kpts_coverage_mask, make_kpts_heatmask,
                                 show_coverage_map, testdata_coverage,
                                 warp_patch_onto_kpts, warped_patch_generator,)
from vtool.depricated import (ThumbnailCacheContext,)
from vtool.distance import (L1, L2, L2_root_sift, L2_sift, L2_sift_sqrd,
                            L2_sqrd, TAU, TEMP_VEC_DTYPE, VALID_DISTS,
                            bar_L2_sift, bar_cos_sift, closest_point,
                            compute_distances, cos_sift, cosine_dist,
                            cyclic_distance, det_distance, emd, haversine,
                            hist_isect, nearest_point, ori_distance,
                            pdist_argsort, pdist_indicies, safe_pdist,
                            signed_cyclic_distance, signed_ori_distance,
                            testdata_hist, testdata_sift2,
                            understanding_pseudomax_props, wrapped_distance,)
from vtool.ellipse import (adaptive_scale, check_kpts_in_bounds,
...
from vtool.fontdemo import (Bitmap, Font, Glyph, font_demo, get_text_test_img,)
from vtool.geometry import (bbox_center, bbox_from_center_wh, bbox_from_extent,
...
from vtool.histogram import (TAU, argsubextrema2, argsubmax, argsubmax2,
...
from vtool.image import (EXIF_TAG_DATETIME, EXIF_TAG_GPS, LINE_AA, TAU,
                         ...
                         warpHomog,)
from vtool.image_filters import (IntensityPreproc, adapteq_fn, clean_mask,
                                 grabcut_fn, histeq_fn, manta_matcher_filters,
                                 medianfilter_fn,)
from vtool.image_shared import (open_pil_image, print_image_checks,)
from vtool.inspect_matches import (INSPECT_BASE, MatchInspector,
                                   MatplotlibWidget, lazy_test_annot,
                                   make_match_interaction, show_matching_dict,)
from vtool.keypoint import (GRAVITY_THETA, KPTS_DTYPE, LOC_DIMS, ORI_DIM,
                            SCAX_DIM, SCAY_DIM, SHAPE_DIMS, SKEW_DIM, TAU,
                            XDIM, YDIM, augment_2x2_with_translation,
                            cast_split, convert_kptsZ_to_kpts,
                            ...
                            transform_kpts_xys,)
from vtool.linalg import (TAU, TRANSFORM_DTYPE, add_homogenous_coordinate,
...
from vtool.matching import (AnnotPairFeatInfo, AssignTup, MatchingError,
                            ...
                            PairwiseMatch, SUM_OPS, TAU, VSONE_ASSIGN_CONFIG,
...
from vtool.other import (and_lists, argsort_groups, argsort_records,
                         ...
                         atleast_nd, atleast_shape, axiswise_operation2,
...
from vtool.patch import (GaussianBlurInplace, TAU, draw_kp_ori_steps,
...
from vtool.quality_classifier import (compute_average_contrast,
                                      contrast_measures, fourier_devtest,
                                      test_average_contrast,)
...
from vtool.segmentation import (DEBUG_SEGM, clean_mask, demo_grabcut,
                                fill_holes, from_hsv_flags, grabcut, grabcut2,
                                into_hsv_flags, mask_colored_img, printDBG,
                                resize_img_and_bbox, segment,)
from vtool.spatial_verification import (HAVE_SVER_C_WRAPPER, INDEX_DTYPE,
                                        SV_DTYPE, TAU, VERBOSE_SVER,
...
from vtool.sver_c_wrapper import (FLAGS_RO, FLAGS_RW, TAU, c_double_p,
                                  call_hello, dpath, errs_t, fm_dtype, fm_t,
                                  fs_dtype, fs_t, get_affine_inliers_cpp,
                                  get_best_affine_inliers_cpp, inliers_t,
                                  kpts_dtype, kpts_t, lib_fname, mats_t,
                                  test_sver_wrapper, test_sver_wrapper2,)
from vtool.symbolic import (check_expr_eq, custom_sympy_attrs, evalprint,
                            symbolic_randcheck, sympy_latex_repr, sympy_mat,
                            sympy_numpy_repr,)
from vtool.trig import (TAU, atan2,)
from vtool.util_math import (TAU, beaton_tukey_loss, beaton_tukey_weight,
...
__all__ = ['AnnotPairFeatInfo', 'AnnoyWraper', 'AnnoyWrapper', 'AssignTup',
           'Bitmap', 'COVGRID_DEFAULT', 'COVKPTS_DEFAULT', 'ConfusionMetrics',
           'DATETIMEORIGINAL_TAGID', 'DEBUG_SEGM', 'EXIF_TAG_DATETIME',
           'EXIF_TAG_GPS', 'EXIF_TAG_TO_TAGID', 'FLAGS_RO', 'FLAGS_RW', 'Font',
           'GPSDATE_CODE', 'GPSINFO_CODE', 'GPSLATITUDEREF_CODE',
           'GPSLATITUDE_CODE', 'GPSLONGITUDEREF_CODE', 'GPSLONGITUDE_CODE',
           'GPSTIME_CODE', 'GPS_TAG_TO_GPSID', 'GRAVITY_THETA',
           'GaussianBlurInplace', 'Glyph', 'HAVE_SVER_C_WRAPPER',
           'INDEX_DTYPE', 'INSPECT_BASE', 'IntensityPreproc', 'KPTS_DTYPE',
           'L1', 'L2', 'L2_root_sift', 'L2_sift', 'L2_sift_sqrd', 'L2_sqrd',
           'LINE_AA', 'LOC_DIMS', 'MatchInspector', 'MatchingError',
           'MatplotlibWidget', 'NORM_CHIP_CONFIG', 'ORIENTATION_000',
           'ORIENTATION_090', 'ORIENTATION_180', 'ORIENTATION_270',
           'ORIENTATION_CODE', 'ORIENTATION_DICT', 'ORIENTATION_DICT_INVERSE',
           ...
           'ScoreNormalizer', 'TAU', 'TAU', 'TAU', 'TAU', 'TAU', 'TAU', 'TAU',
           'TAU', 'TAU', 'TAU', 'TAU', 'TEMP_VEC_DTYPE', 'TRANSFORM_DTYPE',
           'ThumbnailCacheContext', 'VALID_DISTS', 'VERBOSE_SVER',
           'VSONE_ASSIGN_CONFIG', 'VSONE_DEFAULT_CONFIG', 'VSONE_FEAT_CONFIG',
           'VSONE_PI_DICT', 'VSONE_RATIO_CONFIG', 'VSONE_SVER_CONFIG', 'XDIM',
           'YDIM', 'adapteq_fn', 'adaptive_scale', 'add_homogenous_coordinate',
           ...
           'augment_2x2_with_translation', 'axiswise_operation2',
           'bar_L2_sift', 'bar_cos_sift', 'bbox_center', 'bbox_from_center_wh',
           'bbox_from_extent', 'bbox_from_verts', 'bbox_from_xywh',
           'bboxes_from_vert_list', 'beaton_tukey_loss', 'beaton_tukey_weight',
           'blend', 'blend_images', 'blend_images_average',
           'blend_images_average_stack', 'blend_images_mult_average',
           'blend_images_multiply', 'breakup_equal_streak',
           'build_affine_lstsqrs_Mx6', 'build_lstsqrs_Mx9', 'c_double_p',
           'calc_error_bars_from_sample', 'calc_sample_from_error_bars',
           'call_hello', 'cast_split', 'check_exif_keys', 'check_expr_eq',
           'check_kpts_in_bounds', 'check_sift_validity',
           'check_unused_kwargs', 'chip', 'circular_distance', 'clean_mask',
           'clean_mask', 'clipnorm', 'clipwhite', 'clipwhite_ondisk',
           'closest_point', 'closest_point_on_bbox', 'closest_point_on_line',
           ...
           'compute_affine', 'compute_average_contrast', 'compute_chip',
           'compute_distances', 'compute_homog',
           'compute_ndarray_unique_rowids_unsafe',
           'compute_subbin_to_bins_dist', 'compute_unique_arr_dataids',
           'compute_unique_data_ids', 'compute_unique_data_ids_',
           'compute_unique_integer_data_ids', 'confusion', 'contrast_measures',
           'convert_colorspace', 'convert_degrees',
           'convert_image_list_colorspace', 'convert_kptsZ_to_kpts',
           'cos_sift', 'cosine_dist', 'coverage_grid', 'coverage_kpts',
           'crop_out_imgfill', 'csum', 'custom_sympy_attrs', 'cvt_BGR2L',
           'cvt_BGR2RGB', 'cvt_bbox_xywh_to_pt1pt2', 'cyclic_distance',
           'decompose_Z_to_RV_mats2x2', 'decompose_Z_to_V_2x2',
           'decompose_Z_to_invV_2x2', 'decompose_Z_to_invV_mats2x2',
           'demo_grabcut', 'demodata_match', 'depricated', 'det_distance',
           'det_ltri', 'detect_opencv_keypoints', 'distance',
           'distance_to_lineseg', 'dot_ltri', 'dpath', 'draw_border',
           'draw_kp_ori_steps', 'draw_precision_recall_curve',
           'draw_roc_curve', 'draw_text', 'draw_verts', 'ellipse',
           ...
           'ensure_shape', 'eps', 'errs_t', 'estimate_pdf',
           'estimate_refined_transform', 'evalprint', 'example_binary', 'exif',
           'expand_kpts', 'expand_scales', 'expand_subscales',
           'extent_from_bbox', 'extent_from_verts', 'extract_chip_from_gpath',
           'extract_chip_from_gpath_into_square', 'extract_chip_from_img',
           'extract_chip_into_square', 'extract_feature_from_patch',
           'extract_features', 'extrema_neighbors', 'features', 'fill_holes',
           ...
           'flatten_scores', 'fm_dtype', 'fm_t', 'font_demo', 'fontdemo',
           'fourier_devtest', 'from_hsv_flags', 'fromiter_nd', 'fromiter_nd',
           'fs_dtype', 'fs_t', 'gamma_adjust', 'gauss2d_pdf', 'gauss_func1d',
           ...
           'get_affine_inliers_cpp', 'get_best_affine_inliers',
           'get_best_affine_inliers_', 'get_best_affine_inliers_cpp',
           'get_coverage_grid_gridsearch_configs',
           'get_coverage_kpts_gridsearch_configs', 'get_covered_mask',
           'get_crop_slices', 'get_cross_patch', 'get_even_point_sample',
           'get_exif_dict', 'get_exif_dict2', 'get_exif_tagids', 'get_exist',
           'get_extract_features_default_params', 'get_extramargin_measures',
           'get_flann_cfgstr', 'get_flann_fpath', 'get_flann_params',
           'get_flann_params_cfgstr', 'get_gaussian_weight_patch',
           'get_grid_kpts', 'get_histinfo_str', 'get_image_to_chip_transform',
           'get_invVR_mats2x2', 'get_invVR_mats3x3', 'get_invVR_mats_oris',
           'get_invVR_mats_shape', 'get_invVR_mats_sqrd_scale',
           'get_invVR_mats_xys', 'get_invV_mats', 'get_invV_mats2x2',
           'get_invV_mats3x3', 'get_invVs', 'get_kdtree_flann_params',
           ...
           'get_stripe_patch', 'get_subbin_xy_neighbors', 'get_test_patch',
           'get_text_test_img', 'get_transforms_from_patch_image_kpts',
           'get_uncovered_mask', 'get_undirected_edge_ids',
           'get_uneven_point_sample', 'get_unixtime', 'get_unixtime_gps',
           'get_unwarped_patch', 'get_unwarped_patches', 'get_warped_patch',
           'get_warped_patches', 'get_xy_strs', 'get_xys', 'grab_webcam_image',
           'grabcut', 'grabcut2', 'grabcut_fn', 'gradient_fill',
           'gradient_magnitude', 'greedy_setcover', 'gridsearch_addWeighted',
           'gridsearch_chipextract', 'gridsearch_coverage_grid',
           'gridsearch_coverage_grid_mask', 'gridsearch_image_function',
           'gridsearch_kpts_coverage_mask', 'group_consecutive',
           'group_indices', 'groupby', 'groupby_dict', 'groupby_gen',
           'groupedzip', 'haversine', 'hist_argmaxima', 'hist_argmaxima2',
           'hist_edges_to_centers', 'hist_isect', 'histeq_fn', 'histogram',
           'homogenous_circle_pts', 'iceil', 'image', 'image_filters',
           'image_shared', 'imread', 'imread_remote_s3', 'imread_remote_url',
           'imwrite', 'imwrite_fallback', 'inbounds', 'index_partition',
           'index_to_boolmask', 'infer_vert', 'inliers_t', 'inspect_matches',
           'inspect_pdfs', 'interact_roc_factory', 'intern_warp_single_patch',
           ...
           'into_hsv_flags', 'inv_ltri', 'invert_apply_grouping',
           'invert_apply_grouping2', 'invert_apply_grouping3',
           'invert_invV_mats', 'inverted_sift_patch', 'invertible_stack',
           'invsum', 'iround', 'iter_reduce_ufunc', 'iter_reduce_ufunc',
           'jagged_group', 'keypoint', 'kp_cpp_infostr', 'kpts_docrepr',
           'kpts_dtype', 'kpts_matrices', 'kpts_repr', 'kpts_t',
           'lazy_test_annot', 'learn_score_normalization', 'lib_fname',
           'linalg', 'linear_interpolation', 'list_compress_', 'list_take_',
           'logistic_01', 'logit', 'make_channels_comparable',
           'make_exif_dict_human_readable', 'make_grid_coverage_mask',
           'make_heatmask', 'make_kpts_coverage_mask', 'make_kpts_heatmask',
           'make_match_interaction', 'make_test_image_keypoints', 'make_video',
           'make_video2', 'make_white_transparent', 'manta_matcher_filters',
           'mask_colored_img', 'matching', 'mats_t', 'maxima_neighbors',
           'maximum_parabola_point', 'median_abs_dev', 'medianfilter_fn',
           ...
           'perlin_noise', 'plot_centroids', 'plot_postbayes_pdf',
           'plot_prebayes_pdf', 'point_inside_bbox', 'printDBG',
           'print_image_checks', 'quality_classifier', 'random_affine_args',
           ...
           'resize_img_and_bbox', 'resize_mask', 'resize_thumb',
           'resize_to_maxdims', 'resize_to_maxdims_ondisk',
           'resized_clamped_thumb_dims', 'resized_dims_and_ratio',
           'rotate_image', 'rotate_image_ondisk',
           'rotation_around_bbox_mat3x3', 'rotation_around_mat3x3',
           ...
           'score_normalization', 'segment', 'segmentation', 'shear',
           'shear_mat3x3', 'show_coverage_grid', 'show_coverage_map',
           'show_gaussian_patch', 'show_hist_submaxima', 'show_matching_dict',
           'show_ori_image', 'show_ori_image_ondisk',
           'show_patch_orientation_estimation', 'signed_cyclic_distance',
           'signed_ori_distance', 'significant_shape', 'sorted_indices_ranges',
           'sparse_grid_coverage', 'spatial_verification',
           'spatially_verify_kpts', 'stack_image_list',
           ...
           'sver_c_wrapper', 'symbolic', 'symbolic_randcheck',
           'symmetric_correspondence', 'sympy_latex_repr', 'sympy_mat',
           'sympy_numpy_repr', 'take2', 'take_col_per_row',
           'test_affine_errors', 'test_annoy', 'test_average_contrast',
           ...
           'test_show_gaussian_patches2', 'test_sver_wrapper',
           'test_sver_wrapper2', 'testdata_annot_metadata', 'testdata_blend',
           'testdata_coverage', 'testdata_hist', 'testdata_imglist',
           'testdata_matching_affine_inliers',
           'testdata_matching_affine_inliers_normalized', 'testdata_patch',
           'testdata_score_normalier', 'testdata_scores_labels',
           'testdata_sift2', 'tests', 'testshow_extramargin_info',
           ...
           'warp_patch_onto_kpts', 'warped_patch_generator',
           'weighted_average_scoring', 'weighted_gaussian_falloff',
           'weighted_geometic_mean', 'weighted_geometic_mean_unnormalized',
           'whiten_xy_points', 'wrap_histogram', 'wrapped_distance', 'zipcat',
           'zipcompress', 'zipcompress_safe', 'ziptake', 'zstar_value']
# </AUTOGEN_INIT>
